...
def _build_mega_query(kpi_ids: list[str]) -> tuple[str, str]:
    """Combine query templates from multiple KPIs into one broad query + themes."""
    ...
    for kid in kpi_ids:
        ...
        if not nq:
            continue
        for term in nq.signal_terms:
            all_terms.add(f'"{term}"' if " " in term else term)
        for theme in nq.themes.split(","):
            t = theme.strip()
            if t:
                all_themes.add(t)

    query_body = " OR ".join(sorted(all_terms))
    query_template = f"({query_body}) AND ({{country}})"
    themes = ",".join(sorted(all_themes))
    return query_template, themes
...
def fetch_news_bulk(
    ...
    """Single Newscatcher call covering all KPIs for the full data date range.

    Returns raw articles; callers should run synthesize_news_context to score
    and condense them.
    """
    valid_kpi_ids = [k for k in kpi_ids if k in KPI_NEWS_QUERIES]
    ...
    cache_key = ("__bulk__", sorted_countries, tuple(sorted(valid_kpi_ids)),
                 f"{from_date}_{to_date}")
    cached = _news_cache.get(cache_key)
    if cached is not None:
        return list(cached)

    query_template, themes = _build_mega_query(valid_kpi_ids)
    resolved_query = _resolve_country_placeholder(query_template, countries)
    ...
    log.info("[NC-BULK] q=%s, from=%s, to=%s, page_size=%s",
             resolved_query[:120], from_date, to_date, page_size)
    try:
        resp = http_requests.post(
            NEWSCATCHER_URL, headers=headers, json=payload,
            timeout=NEWSCATCHER_TIMEOUT,
        )
        if not resp.ok:
            log.warning("[NC-BULK] %s — %s", resp.status_code, resp.text[:500])
            return []
        data = resp.json()
        ...
        _news_cache.put(cache_key, articles)
        log.info("[NC-BULK] fetched %d articles in one call", len(articles))
        return articles
    except Exception as exc:
        log.warning("[NC-BULK] call failed: %s", exc)
        return []
